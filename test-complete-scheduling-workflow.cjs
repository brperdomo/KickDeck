 ...
 * for the SCHEDULING TEAMS event (ID: 1656618593) with available teams.
 * 
 * Steps:
...
async function testCompleteSchedulingWorkflow() {
  ...
  try {
    // Use direct API calls to get data
    const eventResponse = await fetch(`http://localhost:5000/api/admin/events/${EVENT_ID}`);
    ...
    const teamsResponse = await fetch(`http://localhost:5000/api/admin/teams?eventId=${EVENT_ID}`);
    const teamsData = await teamsResponse.json();
    const allTeams = Array.isArray(teamsData) ? teamsData : (teamsData?.teams || []);
    
    const approvedTeams = allTeams.filter(teamData => {
      const team = teamData.team || teamData;
      return team.status === 'approved';
    });
    
    console.log(`\n👥 Total Approved Teams: ${approvedTeams.length}`);
    
    if (approvedTeams.length === 0) {
      console.log('❌ No approved teams found for this event');
      return;
    }
    
    // Group teams by age group
    const teamsByAgeGroup = {};
    approvedTeams.forEach(teamData => {
      const team = teamData.team || teamData;
      const ageGroup = teamData.ageGroup || { name: 'Unknown Age Group' };
      
      if (!teamsByAgeGroup[ageGroup.name]) {
        teamsByAgeGroup[ageGroup.name] = [];
      }
      teamsByAgeGroup[ageGroup.name].push({ team, ageGroup });
    });
    
    console.log('\n📊 Teams by Age Group:');
    Object.entries(teamsByAgeGroup).forEach(([ageGroupName, teams]) => {
      console.log(`  - ${ageGroupName}: ${teams.length} teams`);
    });
    
    // Find age group with most teams for testing
    let testAgeGroup = null;
    let testTeams = [];
    let maxTeams = 0;
    ...
    Object.entries(teamsByAgeGroup).forEach(([ageGroupName, teams]) => {
      if (teams.length > maxTeams) {
        ...
        testAgeGroup = ageGroupName;
        testTeams = teams;
      }
    });
    
    if (testTeams.length < 2) {
      console.log('❌ Need at least 2 teams to create a tournament schedule');
      return;
    }
    
    console.log(`\n🎯 Using ${testAgeGroup} with ${testTeams.length} teams for workflow test`);
    testTeams.forEach((teamData, index) => {
      console.log(`  ${index + 1}. ${teamData.team.name} (ID: ${teamData.team.id})`);
    });
    
    // Limit to 4 teams for manageable testing
    const workflowTeams = testTeams.slice(0, Math.min(4, testTeams.length));
    console.log(`\n⚽ Using ${workflowTeams.length} teams for workflow test`);
    
    await runSchedulingWorkflow(event, testAgeGroup, workflowTeams);
...
async function runSchedulingWorkflow(event, ageGroupName, teams) {
  ...
  console.log('==============================');
  console.log(`Event: ${event.name}`);
  console.log(`Age Group: ${ageGroupName}`);
...
async function step1_FlightTeams(workflowData) {
  const { teams, ageGroupName } = workflowData;
  
  console.log(`Creating flight for ${teams.length} teams in ${ageGroupName}`);
  
  // Create a single flight containing all teams
  const flight = {
    id: `flight_${ageGroupName.replace(/\s+/g, '_')}_1`,
    name: `${ageGroupName} Flight 1`,
    ageGroupName: ageGroupName,
    teams: teams.map(teamData => ({
      id: teamData.team.id,
      name: teamData.team.name,
      ageGroupId: teamData.ageGroup?.id || 'unknown',
      flightId: `flight_${ageGroupName.replace(/\s+/g, '_')}_1`
    })),
  ...
  teams.forEach(teamData => {
    console.log(`    - ${teamData.team.name} → ${flight.name}`);
  });
