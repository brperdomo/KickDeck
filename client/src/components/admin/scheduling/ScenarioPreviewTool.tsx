...
export function ScenarioPreviewTool({ eventId, baselineData, onScenarioSelect }: ScenarioPreviewToolProps) {
  ...
  const calculateScenario = async (params: ScenarioParams) => {
    ...
    try {
      ...
      const result = await simulateScenarioImpact(params);
      setCurrentScenario(result);
      
      // Store scenario for comparison
      const scenarioKey = `${params.bufferTime}-${params.gameDuration}-${params.fieldCount}`;
      setScenarios(prev => ({
        ...prev,
        [scenarioKey]: result
      }));
  ...
  const simulateScenarioImpact = async (params: ScenarioParams): Promise<ScenarioResult> => {
    // Estimate total games from baseline data
    const teamCount = baselineData.teams?.length || 16;
    const estimatedGamesPerTeam = 4; // Average for tournament play
    const totalGames = Math.floor((teamCount * estimatedGamesPerTeam) / 2); // Each game has 2 teams
