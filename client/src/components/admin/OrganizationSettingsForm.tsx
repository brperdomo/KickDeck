...
const formSchema = z.object({
  ...
  primaryColor: z.string().regex(/^#([A-Fa-f0-9]{6})$/, "Invalid color format"),
  secondaryColor: z.string().regex(/^#([A-Fa-f0-9]{6})$/, "Invalid color format"),
});
...
export function OrganizationSettingsForm() {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      // Import Vibrant using dynamic import for color extraction
      ...
      updatePreview({
        ...
        secondaryColor: palette.LightVibrant?.hex || form.getValues('secondaryColor'),
      });
