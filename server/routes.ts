...
export function registerRoutes(app: Express): Server {
  ...
  try {
    ...
    app.get('/api/admin/sendgrid/templates', isAdmin, async (req: Request, res: Response) => {
      try {
    ...
    app.get('/api/admin/sendgrid/template-mappings', isAdmin, async (req: Request, res: Response) => {
      try {
    ...
    app.post('/api/admin/sendgrid/template-mapping', isAdmin, async (req: Request, res: Response) => {
      try {
        ...
        // Update the template mapping in the database
        ...
        res.status(500).json({ error: 'Failed to map template' });
      }
    });
    
    // Get SendGrid provider status
    app.get('/api/admin/sendgrid/settings', isAdmin, async (req: Request, res: Response) => {
      try {
        ...
        if (!response.ok) {
          return res.json({
            ...
            apiKeySet: true,
            apiKeyValid: false,
            message: `SendGrid API key is invalid: ${response.status} ${response.statusText}`
          });
        }
        ...
        res.status(500).json({ 
          configured: false, 
          error: 'Failed to verify SendGrid configuration' 
        });
      }
    });
    
    // Send a test email using a SendGrid template
    app.post('/api/admin/sendgrid/test-template', isAdmin, async (req: Request, res: Response) => {
      try {
        ...
        if (!process.env.SENDGRID_API_KEY) {
          return res.status(500).json({ error: 'SENDGRID_API_KEY environment variable is not set' });
        }
        
        // Directly call the SendGrid API to send a test email
