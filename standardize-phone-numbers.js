 ...
 * 
 * The script safely formats phone numbers without losing data and provides detailed logging.
 */
...
function formatPhoneNumber(phoneNumber) {
  ...
  if (cleaned.length === 10) {
    ...
    // 7-digit local number - format as best we can
    ...
    for (const team of teamsWithManagerPhones) {
      ...
      if (originalPhone !== formattedPhone) {
        try {
          ...
          console.error(`❌ Failed to update team ${team.id}: ${error.message}`);
        }
      } else {
        skippedCount++;
      }
  ...
  try {
    ...
    for (const team of teamsWithCoaches) {
      try {
        ...
        console.error(`❌ Failed to process coach data for team ${team.id}: ${error.message}`);
        skippedCount++;
      }
    }
    ...
    for (const player of playersWithPhones) {
      ...
      if (hasChanges) {
        try {
          ...
          console.error(`❌ Failed to update player ${player.id}: ${error.message}`);
        }
      } else {
        skippedCount++;
      }
...
export { formatPhoneNumber, standardizeUserPhones, standardizeTeamManagerPhones, standardizeCoachPhones, standardizePlayerPhones };